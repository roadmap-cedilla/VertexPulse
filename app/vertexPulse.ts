...
export default function VertexPulse() {
  ...
  async function runProbe() {
    ...
    if (isAddress(probeAddress)) {
      const bal = await client.getBalance({ address: probeAddress });
      setBalance(bal);
    } else {
      setBalance(null);
    }
  ...
  return (
    <OnchainKitProvider chain={chain}>
      <div style={{ maxWidth: 860, margin: "48px auto", fontFamily: "system-ui" }}>
        <h1>VertexPulse — Built for Base</h1>
